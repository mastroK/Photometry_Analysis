...
RED_L_FIG_DIR = Path("figures_fixed_model_series_sm_red_l_truncated/final")
GREEN_L_FIG_DIR = Path("figures_fixed_model_series_sm_green_l_truncated/final")
OUT_DIR = RED_L_FIG_DIR

COLOR_RED = "#B8763E"
COLOR_GREEN = "#3B5B74"

MIN_RESID_DOF = 10  # must match models.glm_encoding.fit_time_resolved_glm's default


def _mask_degenerate_timepoints(df, min_resid_dof=MIN_RESID_DOF):
    """These truncated beta_df.csv files were saved by a run of plot_pooled
    that predates the min_resid_dof guard landing in fit_time_resolved_glm
    (see models/glm_encoding.py) -- confirmed by comparing file mtimes, not
    a coincidence. Rather than pay for a fresh ~85 min session-reload just to
    re-fit, reapply the identical rule post hoc: NaN out any timepoint whose
    residual degrees of freedom (n_trials - n_params) fall below
    min_resid_dof, exactly as the fixed fit_time_resolved_glm would have.
    Legitimate because it's the same criterion computed from data already in
    the CSV -- not a new assumption -- and every affected timepoint here is
    deep in the truncation-diagnostic's known near-empty tail (>4s, where
    <0.03% of trials still have unmasked signal) rather than the
    scientifically meaningful 0-3s range.
    """
    beta_cols = [c for c in df.columns if c.endswith("_beta")]
    n_params = len(beta_cols)
    df_resid = df["n_trials"] - n_params
    degenerate = df_resid < min_resid_dof
    df = df.copy()
    value_cols = [c for c in df.columns if c != "n_trials"]
    df.loc[degenerate, value_cols] = np.nan
    if degenerate.any():
        print(f"  masked {degenerate.sum()}/{len(df)} degenerate timepoint(s) "
              f"(n_params={n_params}, min_resid_dof={min_resid_dof})")
    return df
...
def make_overlay(model_name, event="side_in"):
    ...
    df_red = _mask_degenerate_timepoints(pd.read_csv(red_path, index_col=0))
    df_green = _mask_degenerate_timepoints(pd.read_csv(green_path, index_col=0))
